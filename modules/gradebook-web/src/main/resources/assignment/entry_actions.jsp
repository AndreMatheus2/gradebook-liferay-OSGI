<%@ include file="/init.jsp"%><c:set var="assignment" value="${SEARCH_CONTAINER_RESULT_ROW.object}" />
...
<%-- Permissions action. --%><c:if test="${assignmentPermission.contains(permissionChecker, assignment.assignmentId,
...
<%-- Delete action. --%>
<c:if test="${assignmentPermission.contains(permissionChecker, assignment.assignmentId,
<portlet:actionURL name="<%=MVCCommandNames.DELETE_ASSIGNMENT %>" var="deleteAs
<portlet:param name="redirect" value="${currentURL}" />
<portlet:param name="assignmentId" value="${assignment.assignmentId}" /
</portlet:actionURL>
